            ...
            <div className="overflow-x-auto -mx-6">
              ...
              <div className="sm:hidden space-y-2">
                {income.map((incomeItem) => (
                  <div key={incomeItem.id} className="border rounded-lg p-3 space-y-1.5 hover:bg-muted/20 transition-colors">
                    <div className="flex items-start justify-between gap-2">
                      <span className="text-sm font-medium">{incomeItem.descricao}</span>
                      <span className="text-sm font-bold text-success flex-shrink-0">{formatCurrency(incomeItem.valor)}</span>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium ${
                        ...
                        {incomeItem.tipo === 'salario' ? 'Salário' : incomeItem.tipo === 'adiantamento' ? 'Adiantamento' : 'Outro'}
                      </span>
                      <span className="text-xs text-muted-foreground">Dia {incomeItem.dia_recebimento}</span>
                    ...
                    <div className="flex justify-end gap-1 pt-1 border-t border-border/40">
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openDialog(incomeItem)}>
                        <Edit className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-7 w-7 hover:text-destructive" onClick={() => handleDelete(incomeItem)}>
